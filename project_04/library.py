...
# Sidebar Menu
menu = st.sidebar.radio("📌 Menu", ["Add Book", "Remove Book", "Search Book", "Display Books", "Favorite Books", "Statistics"])
...
if menu == "Add Book":
    ...
    if st.button("Search"):
        ...
        if results:
            for book in results:
                ...
                fav_status = "⭐ Favorite" if book["favorite"] else ""
                st.write(f"📖 **{book['title']}** by {book['author']} ({book['year']}) - {book['genre']} - {status} {fav_status}")
        else:
    ...
    if library:
        for i, book in enumerate(library, start=1):
            status = "✅ Read" if book["read"] else "❌ Unread"
            fav_status = "⭐ Favorite" if book["favorite"] else ""
            st.write(f"{i}. 📖 **{book['title']}** by {book['author']} ({book['year']}) - {book['genre']} - {status} {fav_status}")
    else:
        st.warning("📭 No books in the library!")

elif menu == "Favorite Books":
    st.subheader("⭐ Favorite Books")
    fav_books = [book for book in library if book["favorite"]]
    if fav_books:
        for i, book in enumerate(fav_books, start=1):
            status = "✅ Read" if book["read"] else "❌ Unread"
            st.write(f"{i}. 📖 **{book['title']}** by {book['author']} ({book['year']}) - {book['genre']} - {status}")
    else:
        st.warning("💔 No favorite books yet!")
